...
class ShapeShift:
    ...
    def tx_by_apikey(self, apiKey):
        ...
        Transactions are created with an affilliate PUBLIC KEY,
        but they are looked up using the linked PRIVATE KEY, to protect the privacy of our affiliates' account details.
    ...
    def tx_by_address(self, apiKey, address):
        ...
        The affilliate's PRIVATE KEY must be provided,
        and will only return transactions that were sent to output address AND
        were created using / linked to the affiliate's PUBLIC KEY.
        Please note that if the address is a ripple address and it includes the "?dt=destTagNUM" appended on the end,
        you will need to use the URIEncodeComponent() function on the address before sending it in as a param,
        to get a successful response.
    ...
    def validate_address(self, address, coin):
        ...
        return self.get_request(self.url)['isvalid']
...
if __name__ == '__main__':
    a = ShapeShift()
    eb = a.market_info('eth_btc')
    be = a.market_info(('btc_eth'))
    print(eb)
    print(be)
